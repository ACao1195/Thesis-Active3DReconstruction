/* get_tf_RGBD_data.cpp
Andrew Yuanheng Cao - 10 March 2020
  Adapted from ORB-SLAM2 - RGBDNode */

#include "../include/get_tf_RGBD_data.h"

// Initalise bag globally (not recommended for larger projects)
rosbag::Bag bag;

void ImageCallback(const sensor_msgs::ImageConstPtr& image, const tf2_msgs::TFMessageConstPtr& tf)
{
  // Write into bag
  bag.write("/camera/rgb/image_raw", ros::Time::now(), image);
  bag.write("/tf", ros::Time::now(), tf);
...
int main(int argc, char **argv)
{
    ...
    // Initialise bag
    bag.open("test1.bag", rosbag::bagmode::Write);

    // Create subscriber lines for the raw image and tf
    message_filters::Subscriber<sensor_msgs::Image> image_sub(node_handle, "/camera/rgb/image_raw", 1);

    message_filters::Subscriber<tf2_msgs::TFMessage> tf_sub(node_handle, "/tf", 1);

    // Combine them into one synchronised subscriber line
